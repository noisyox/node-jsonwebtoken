...
exports.generate = function (header, payload, key, cb) {

	var alg = header.alg
	var enc = header.enc

	var algKey = typeof alg === 'string' ? alg.match(ALG_RE) : null;
	var eKey = typeof enc === 'string' ? enc.match(ENC_RE) : null;
...
exports.parse = function (parts) {
	return new ParsedToken(parts);
}

function ParsedToken(parts) {
	this.parts = parts;
	try {
		this.header = JSON.parse(Buffer.from(this.parts[0], 'base64'));
	} catch (error) {
		this.error = {
			message: `Non parsable header. ${error.message}`
		};
		return;
	}
}
...
ParsedToken.prototype.verify = function (p0, cb) {
	...
	if (p0.constructor !== Object) {
		...
		this.error = {
			message: 'Missing kid claim in header'
		};
	...
	return verifyToken(this, key, cb);
}

function verifyToken(parsed, key, cb) {
	if (parsed.error) {
	...
	if (typeof parsed.header.alg !== 'string') {
		parsed.error = {
			message: 'Missing or invalid alg claim in header'
		};
		return responder(null, parsed, cb);
	}
	if (typeof parsed.header.enc !== 'string') {
		parsed.error = {
			message: 'Missing or invalid enc claim in header'
		};
		return responder(null, parsed, cb);
	}
	...
	if (aMatch[2]) {
		...
		if (!Number.isInteger(iter) || iter < 1 || iter > 10000) {
			parsed.error = {
				message: 'Missing or invalid p2c claim in header'
			};
			return responder(null, parsed, cb);
		} else if (!cb && iter > 1000) {
			parsed.error = {
				message: 'p2c value too large for synchronous mode'
			};
			return responder(null, parsed, cb);
		}
		if (typeof parsed.header.p2s !== 'string') {
			parsed.error = {
				message: 'Missing or invalid p2s claim in header'
			};
			return responder(null, parsed, cb);
		}
...
function decryptJwe(parsed, aMatch, eMatch, key, cb) {
	...
	if (aMatch[0] !== 'dir') {
		...
		try {
			...
			parsed.error = {
				message: `Could not decrypt token. ${error.message}`
			};
			return responder(null, parsed, cb);
		}
	} else {
	...
	try {
		...
		parsed.error = {
			message: `Non parsable payload. ${error.message}`
		};
		return responder(null, parsed, cb);
	}
	return responder(null, parsed, cb);
